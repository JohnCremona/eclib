...
inline int eps4(const ZZ& u) // u must be odd; m1pow(eps4(u))=chi4(u)
{
  return (u+1)%4==0;  // so 1 mod 4 gives 0, 
                      //    3 mod 4 gives 1
}

inline int omega8(const ZZ& u) // u must be odd; m1pow(omega8(u))=chi2(u)
{
...
inline int local_hilbert(const ZZ& a, const ZZ& b, const long& p)
{
  return local_hilbert(a,b,ZZ(p));
}
