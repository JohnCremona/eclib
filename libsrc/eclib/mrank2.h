...
private:
  vector<ZZ> badprimes, supp0, supp1;
  vector<ZZ> elsgens0, elsgens1, els2gens0, els2gens1, glsgens0, glsgens1;
  ...
  ZZ e2, e3, e2dash, e3dash, s2, s4, s6;
...
// lim1  is (ZZ) bound on |x|+|z| in naive search
// lim2 is (double) bound on log max {|x|,|z| }, i.e. logarithmic
// sel is selmer_only switch
// second is do-second-descent switch
  Curvedata getEprime() const override {return Eprime;}
  long getselmerprime() const override {return selmer_rank_Eprime;}
  long getselmerphi() const override {return selmer_rank_phi_Eprime;}
  long getselmerphiprime() const override {return selmer_rank_phiprime_E;}
  void listgens();
  void listgens(Curvedata* CD_orig, const ZZ& u, const ZZ& r,
  ...
  vector<Point> getgens() const override {return pointlist;}
private:
  int testquartic(const ZZ& c, const ZZ& d1, const ZZ& d2, int which);
  int second_descent(const ZZ& c, const ZZ& d1, const ZZ& d2, int which);
  void makepoint(const ZZ& c, const ZZ& d1, const ZZ& d2,
		 const ZZ& x, const ZZ& y, const ZZ& z,
		 int which);
  void local_descent(const ZZ& x0);
  void find_elsgens(int which, const ZZ& c, const ZZ& d);
  void find_els2gens(int which, const ZZ& c, const ZZ& d);
  void find_glsgens(int which, const ZZ& c, const ZZ& d);
  void makegens();
};
