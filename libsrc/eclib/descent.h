...
protected:
  Curvedata* the_curve;
  int verbose, certain, success, selmer_only, do_second_descent, do_CT;
  long num_aux;
...
//
// CT only relevant if second_descent=1 and selmer_only=0 (need work)

  rank12(){;}
  rank12(Curvedata* ec,
         int verb=0, int sel=0,
         long firstlim=20, long secondlim=5,
	 long n_aux=-1, int second_descent=1, int CT=0)
    :the_curve(ec), verbose(verb), selmer_only(sel),
     do_second_descent(second_descent),
     num_aux(n_aux), lim1(firstlim), lim2(secondlim), do_CT(CT) {;}
...
private: rank12 * r12;  // does all the work
  int verbose, two_torsion_exists, selmer_only, do_CT;
