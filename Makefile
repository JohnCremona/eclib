...
OPTFLAG  ?=  $(PICFLAG) -g -O3 -DNEW_OP_ORDER -DUSE_PARI_FACTORING
...
clean:
	...
	cd qrank && ${MAKE} clean
	cd g0n && ${MAKE} clean
	if [ -d lib ] ; then cd lib && rm -f *.a *.so *.dylib *.def ; fi
	if [ -d include ] ; then cd include && rm -f *.h ; fi
	if [ -d bin ] ; then cd bin && rm -f * ; fi
